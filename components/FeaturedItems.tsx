...
const baseItems: BaseItem[] = [
  ...
  },
  {
    ...
    tagColor: 'bg-brand-dark text-white',
  },
];
...
export default function FeaturedItems() {
  ...
  return (
    <section className="py-20 md:py-28 bg-white" aria-labelledby="featured-heading">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {items.map(({ image, alt, name, description, price, tag, tagColor }) => (
            <article key={name} className="group flex flex-col overflow-hidden rounded-2xl bg-white border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
              <div className="relative h-52 overflow-hidden bg-amber-50 flex-shrink-0">
                <Image
                  src={image}
                  alt={alt}
                  fill
              ...
              <div className="p-5 flex flex-col flex-1">
                <h3 className="font-poppins font-bold text-lg text-brand-dark mb-2 leading-snug">{name}</h3>
                ...
                <div className="flex items-center justify-between mt-auto">
                  ...
                  <Link
                    href="/menu"
                    className="text-sm font-semibold text-brand-dark hover:text-brand-red transition-colors"
                    aria-label={`See ${name} on the menu`}
                  >
